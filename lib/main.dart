...
class _ExampleCupertinoDownloadButtonState
  ...
  void _openDownload(int index) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AppDetailPage(appIndex: index),
      ),
    );
  }
...
class AppDetailPage extends StatelessWidget {
  ...
  const AppDetailPage({super.key, required this.appIndex});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('App ${appIndex + 1}'),
      ),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.download_done, size: 110, color: Colors.green),
            ...
            Text(
              'Ini halaman baru setelah tekan OPEN',
...
class SimulatedDownloadController extends DownloadController with ChangeNotifier {
  ...
  Future<void> _doSimulatedDownload() async {
    ...
    const stops = [0.0, 0.15, 0.45, 0.8, 1.0];
